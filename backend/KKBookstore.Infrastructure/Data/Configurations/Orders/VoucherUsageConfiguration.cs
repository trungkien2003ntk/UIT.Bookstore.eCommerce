...
internal class VoucherUsageConfiguration : IEntityTypeConfiguration<VoucherUsage>
{
    public void Configure(EntityTypeBuilder<VoucherUsage> builder)
    {
        ...
        builder.HasOne(vu => vu.Voucher).WithMany(v => v.VoucherUsages).HasForeignKey(vu => vu.VoucherId).OnDelete(DeleteBehavior.Cascade);
        builder.HasOne(vu => vu.Order).WithMany().HasForeignKey(vu => vu.OrderId).OnDelete(DeleteBehavior.Restrict);
        builder.HasOne(vu => vu.Customer).WithMany().HasForeignKey(vu => vu.CustomerId).OnDelete(DeleteBehavior.Restrict);
    }
}